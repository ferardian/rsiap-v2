...
const approveSchedule = async () => {
  if (!confirm('Apakah Anda yakin ingin menyetujui dan mempublikasikan jadwal ini? Action ini akan memperbarui Jadwal Final.')) return
...
.employee-info {
  ...
  flex-direction: column;
  position: relative;
  z-index: 21; /* Ensure text is above background */
}
...
/* Read-only cells in approval view */
.shift-cell-readonly {
...
.btn-save {
  background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); /* Blue for Save Draft */
  color: white;
  padding: 0.75rem 1.5rem;
}

.btn-approve {
  background: linear-gradient(135deg, #10b981 0%, #059669 100%); /* Green for Final Approve */
  color: white;
  padding: 0.75rem 1.5rem;
}
...
@media (max-width: 768px) {
  ...
  .jadwal-page {
    height: calc(100vh - 80px); /* Enable fixed height on mobile */
    min-height: 1200px; /* Reset global min-height */
    overflow: hidden; /* Enable internal scroll */
    padding: 0.5rem !important; /* Restore some padding */
  }
  ...
  .filter-group .btn {
    ...
    min-width: 0 !important;
    margin: 0 !important;
    padding: 0.75rem !important; /* Standardize padding */
    font-size: 1rem !important;
