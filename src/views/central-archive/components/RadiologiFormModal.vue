    ...
    <div class="modal-content" @click.stop>
      <div class="modal-header">
        <h3 class="modal-title">
          ...
          {{ isEdit ? 'Edit Berkas Radiologi' : 'Buat Berkas Radiologi' }}
        </h3>
      ...
      <div class="modal-body pb-0">
        <form @submit.prevent="submitForm" id="radiologiForm" class="modern-form">
          ...
          <div class="form-group">
            <label>Perihal Berkas Radiologi <span>*</span></label>
            <input type="text" v-model="formData.perihal" class="form-control" placeholder="Contoh: Pemeliharaan Preventif Alat Rontgen X-Ray" required>
          </div>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" form="radiologiForm" class="btn-submit" :disabled="loading || (isSubmitted && !selectedPj?.nik)">
...
const formatNomorSurat = (data) => {
  ...
  try {
    ...
    const prefix = data.prefix || 'RAD'
    return `${no}/${prefix}/${tglPattern}`
  } catch (e) {
    return `${data.nomor}/${data.prefix || 'RAD'}`
  }
}
...
const submitForm = async () => {
  ...
  try {
    if (props.isEdit) {
      ...
      await berkasRadiologiService.update(identifier, updateData)
      toast.success('Berkas berhasil diperbarui')
    } else {
      await berkasRadiologiService.store(formData.value)
