    ...
    <div class="modal-content" @click.stop>
      <div class="modal-header">
        <h3 class="modal-title">
          ...
          {{ isEdit ? 'Edit Berkas IHT' : 'Buat Berkas IHT' }}
        </h3>
      ...
      <div class="modal-body pb-0">
        <form @submit.prevent="submitForm" id="ihtForm" class="modern-form">
          ...
          <div class="form-group">
            <label>Perihal IHT <span>*</span></label>
            <input type="text" v-model="formData.perihal" class="form-control" placeholder="Contoh: In-House Training Bantuan Hidup Dasar (BHD)" required>
          </div>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" form="ihtForm" class="btn-submit" :disabled="loading || (isSubmitted && !selectedPj?.nik)">
...
const submitForm = async () => {
  ...
  try {
    if (props.isEdit) {
      ...
      await berkasIhtService.update(identifier, updateData)
      toast.success('Berkas berhasil diperbarui')
    } else {
      await berkasIhtService.store(formData.value)
