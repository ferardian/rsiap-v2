    ...
    <div class="modal-content" @click.stop>
      <div class="modal-header">
        <h3 class="modal-title">
          ...
          {{ isEdit ? 'Edit Berkas Komite PMKP' : 'Buat Berkas Komite PMKP' }}
        </h3>
      ...
      <div class="modal-body pb-0">
        <form @submit.prevent="submitForm" id="pmkpForm" class="modern-form">
          ...
          <div class="form-group">
            ...
            <input type="text" v-model="formData.perihal" class="form-control" placeholder="Contoh: Monitoring Mutu Indikator Area Klinis" required>
          </div>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" form="pmkpForm" class="btn-submit" :disabled="loading || (isSubmitted && !selectedPj?.nik)">
...
const formatNomorSurat = (data) => {
  ...
  try {
    ...
    const prefix = data.prefix || 'KOM-PMKP'
    return `${no}/${prefix}/${tglPattern}`
  } catch (e) {
    return `${data.nomor}/${data.prefix || 'KOM-PMKP'}`
  }
}
...
const submitForm = async () => {
  ...
  try {
    if (props.isEdit) {
      ...
      await komitePmkpService.update(identifier, updateData)
      toast.success('Berkas berhasil diperbarui')
    } else {
      await komitePmkpService.store(formData.value)
