      ...
      <div class="hero-content">
        <div class="title-section">
          ...
          <div class="title-text">
            <h1>Berkas IHT (In-House Training)</h1>
            <p>Data Berkas In-House Training Terdaftar RSIA Aisyiyah Pekajangan</p>
          </div>
        </div>
    ...
    <div class="card-modern">
      <div class="table-container">
        ...
        <div v-else class="empty-state">
          ...
          <p>Belum ada data Berkas IHT yang terdaftar atau sesuai kriteria pencarian.</p>
...
import { berkasIhtService } from '@/services/berkasIhtService'
import debounce from 'lodash/debounce'
import { format } from 'date-fns'

// Import Modals
import IhtFormModal from './components/IhtFormModal.vue'
import IhtDetailModal from './components/IhtDetailModal.vue'
...
const loadData = async (page = 1) => {
  ...
  try {
    ...
    const response = await berkasIhtService.search(searchQuery.value, pagination.value.per_page, page, filters)
    ...
    toast.error('Gagal memuat data Berkas IHT')
  } finally {
    loading.value = false
  }
}
...
const parseDate = (dateStr) => {
  ...
  // Replace space with 'T' to ensure it's parsed as an ISO 8601 string in UTC,
...
const formatNomorSurat = (berkas) => {
  ...
  try {
    ...
    const prefix = berkas.prefix || 'IHT'
    return `${no}/${prefix}/${tglPattern}`
  } catch (e) {
    return `${berkas.nomor}/${berkas.prefix || 'IHT'}`
  }
}
...
const executeDelete = async () => {
  ...
  try {
    ...
    await berkasIhtService.delete(identifier)
