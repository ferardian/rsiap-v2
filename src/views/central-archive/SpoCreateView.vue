    ...
    <div class="create-container">
      ...
      <div class="create-header">
        <div class="d-flex align-items-center gap-3">
          ...
          <div>
            <h2 class="mb-0 fw-bold">Tambah SPO Baru</h2>
            <p class="text-muted small mb-0">Isi formulir untuk membuat Standar Prosedur Operasional baru.</p>
          </div>
        </div>
      </div>
          ...
          <div class="card glass-card border-0 shadow-sm">
            ...
            <div class="card-footer bg-white border-top-0 p-4 text-end">
               ...
               <button class="btn btn-primary px-4 fw-bold shadow-sm" @click="submitSpo" :disabled="isSubmitting">
                 <i class="fas fa-save me-2" :class="{'fa-spin fa-spinner': isSubmitting}"></i> Simpan SPO
               </button>
            </div>
          </div>
        </div>
...
onMounted(async () => {
    // Dynamically import quill to fix SSR/Window issues if any
...
const fetchUnits = async () => {
    try {
        ...
        console.log("Units fetched: ", unitsDict.value)
    } catch(err) {
...
const submitSpo = async () => {
    ...
    try {
        const payload = {
            ...
            // XSS Prevention Mapping like in filetrack
        ...
        Swal.fire({
            icon: 'success',
            title: 'Berhasil',
            text: 'Dokumen SPO baru telah disimpan.',
            timer: 1500,
        ...
        console.error(err)
        const msg = err.response?.data?.message || err.message || 'Gagal menyimpan SPO.'
