<template>
  <div class="peneliti-rme-page min-vh-100 p-3 p-md-4">
    <!-- Navbar / Header for Researcher (Sleek Header Bar) -->
    <div class="card navbar-researcher border-0 shadow-sm rounded-4 mb-3 text-white position-relative overflow-hidden">
      ...
      <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 position-relative z-2">
        <div class="d-flex align-items-center gap-3">
          ...
          <div>
            <h5 class="mb-0 fw-extrabold text-white tracking-tight hero-title-text">{{ permohonan.judul_penelitian || permohonan.judul || 'Portal Data Penelitian RME' }}</h5>
            <div class="text-xs text-slate-300 mt-1 d-flex align-items-center gap-2">
              <span>Peneliti: <b class="text-white">{{ permohonan.nama_peneliti || permohonan.peneliti || username }}</b></span>
              <span class="text-slate-400">&bull;</span>
              <span>{{ permohonan.institusi || 'Institusi' }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="d-flex align-items-center gap-2">
          <span class="badge badge-security-glow">
            <i class="fas fa-shield-check me-2 text-emerald-400"></i> Protected & Masked Mode
          </span>
    ...
    <!-- Quick Stats Cards for Researcher (Generous Spacing Crisp Light Theme) -->
    <div class="row g-3 stats-row-container">
        ...
        <div class="card stat-card-modern border-0 shadow-sm rounded-4 h-100 overflow-hidden">
          <div class="card-body d-flex align-items-center gap-3">
            ...
            <div>
              ...
              <div class="stat-value-modern text-slate-900">{{ pagination.total.toLocaleString() }} Pasien</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card stat-card-modern border-0 shadow-sm rounded-4 h-100 overflow-hidden">
          <div class="card-body d-flex align-items-center gap-3">
            ...
            <div>
              ...
              <div class="stat-value-modern text-cyan-600">De-Identified</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card stat-card-modern border-0 shadow-sm rounded-4 h-100 overflow-hidden">
          <div class="card-body d-flex align-items-center gap-3">
            ...
            <div>
              ...
              <div class="stat-value-modern text-emerald-600">≤ 3 Hari Kerja</div>
            </div>
          </div>
        </div>
      </div>
    </div>    <!-- Filter & Search Toolbar (Clean White Toolbar) -->
        ...
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
          ...
          <div class="d-flex align-items-center gap-2">
            ...
            <button class="btn btn-export-excel-modern shadow-sm" @click="exportCsv" :disabled="loading || isExporting || pagination.total === 0">
              <i :class="['fas me-2', isExporting ? 'fa-spinner fa-spin' : 'fa-file-excel']"></i>
    ...
    <!-- Data Table Card (Clean White Scrollable Table) -->
    <div class="card table-card-modern border-0 shadow-sm rounded-4 overflow-hidden mb-0">
            ...
            <tbody>
              ...
              <tr v-for="item in items" :key="item.no_rawat" class="hover-row">
                <td class="ps-3">
                  ...
                  <div class="text-2xs text-slate-500 font-mono mt-0.5">{{ item.no_rkm_medis }}</div>
                </td>
                <td>
                  <div class="d-flex align-items-center gap-3">
                ...
                <td>
                  ...
                  <div class="text-2xs text-slate-600 font-semibold mt-1">{{ item.umur }}</div>
                </td>
                <td>
                  <div class="fw-bold text-slate-900 text-xs">{{ item.nm_poli }}</div>
                  <div class="text-2xs text-slate-500 mt-0.5">{{ item.penjamin }}</div>
                </td>
                <td>
                  <div class="d-flex align-items-center gap-2">
                ...
                <td class="pe-3 text-end">
                  ...
                  <div class="text-2xs text-slate-400 font-mono mt-0.5">{{ item.jam_reg }}</div>
                </td>
                </tr>
            </tbody>
          </table>
      ...
      <div class="card-footer bg-white py-3 px-3.5 border-top d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div class="d-flex align-items-center gap-2">
...
/* Stat Cards Generous Spacing Crisp Light Theme */
.stat-card-modern {
  background: #ffffff;
...
.stat-icon-modern.primary { background: #eff6ff; color: #2563eb; }
.stat-icon-modern.info { background: #ecfeff; color: #0891b2; }
.stat-icon-modern.success { background: #ecfdf5; color: #059669; }
...
/* Scrollable Table Container & Crisp White Theme */
.table-card-modern {
  border-radius: 18px !important;
  background: #ffffff;
  border: 1px solid #e2e8f0 !important;
...
/* Card Footer & Pagination Light Theme */
.table-card-modern .card-footer {
