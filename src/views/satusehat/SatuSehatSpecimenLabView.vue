<template>
  <div class="satusehat-specimen-lab">
      ...
      <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div class="header-text">
          <div class="d-flex align-items-center gap-2 mb-1">
            ...
            <h2 class="page-title mb-0">Specimen Lab <span class="text-primary-emphasis">SatuSehat</span></h2>
          </div>
          <p class="page-subtitle text-muted mb-0">Sinkronisasi data sampel laboratorium ke platform SatuSehat FHIR</p>
        </div>
      ...
      <div class="col-md-3 min-w-stats">
        <div class="card stat-card shadow-sm border-0 h-100 overflow-hidden">
          <div class="stat-card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <p class="stat-label mb-0">Total Specimen</p>
        ...
        <div class="card stat-card shadow-sm border-0 h-100 overflow-hidden">
          <div class="stat-card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              ...
              <h3 class="stat-value mb-0">{{ items.filter(i => i.id_specimen).length }}</h3>
            </div>
          </div>
            ...
            <tbody>
              ...
              <tr v-else-if="items.length === 0">
                <td colspan="6" class="py-5 text-center no-data-state">
                  ...
                  <p class="mt-3 fw-medium text-secondary">No specimen records found</p>
              ...
              <tr v-for="item in items" :key="item.noorder + item.id_template" class="hover-row">
                ...
                <td>
                  ...
                  <div class="order-date"><i class="far fa-clock me-1"></i> {{ item.tgl_sampel }} {{ item.jam_sampel }}</div>
                  <div class="exam-meta">
                    <span class="exam-code">Type: {{ item.sampel_display }}</span>
                  </div>
                </td>
                <td>
                  <div v-if="item.id_servicerequest" class="sync-status success mini">
                    <div class="sync-icon"><i class="fas fa-link"></i></div>
                    <div class="sync-text">
                      <span class="status-id">{{ item.id_servicerequest }}</span>
                    </div>
                  </div>
                  <div v-else class="sync-status danger mini">
                    <div class="sync-icon"><i class="fas fa-unlink"></i></div>
                    <div class="sync-text">
                      <span class="status-label">Missing SR</span>
                    </div>
                  </div>
                </td>
                <td>
                  <transition name="scale" mode="out-in">
                    <div v-if="item.id_specimen" class="sync-status success" :key="'sent-' + item.noorder">
                ...
                <td class="pe-4 text-end">
                  <div class="action-buttons">
                    <button 
                      ...
                      :class="item.id_specimen ? 'btn-update' : 'btn-send'"
                      @click="sendToSatuSehat(item)"
                      :disabled="sendingItems.includes(item.noorder + item.id_template) || !item.no_ktp || !item.id_servicerequest"
                    >
                      ...
                      <template v-else>
                        <i :class="item.id_specimen ? 'fas fa-sync' : 'fas fa-paper-plane'"></i>
                        {{ item.id_specimen ? 'Refetch' : 'Send Specimen' }}
                      </template>
                    </button>
                  </div>
                </td>
...
export default {
  ...
  methods: {
    async fetchData() {
      this.loading = true
      try {
        const res = await satuSehatService.getSpecimenLab(this.filters)
        if (res.data.success) {
    ...
    async syncAll() {
      const result = await Swal.fire({
        ...
        text: `Sistem akan mengirimkan seluruh Specimen Lab yang belum terkirim pada periode registrasi ${this.filters.tgl_awal} s/d ${this.filters.tgl_akhir} ke SatuSehat.`,
        icon: 'question',
      ...
      this.syncing = true
      try {
        const res = await satuSehatService.syncSpecimenLab({
    ...
    async sendToSatuSehat(item) {
      ...
      try {
        const res = await satuSehatService.sendSpecimenLab(item)
        if (res.data.success) {
          Swal.fire({ 
            icon: 'success', 
...
.sync-status.danger .sync-icon { background: #ef4444; color: white; }
...
.sync-status.danger .status-label { color: #991b1b; }
