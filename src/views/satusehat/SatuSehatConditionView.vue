<template>
  <div class="satusehat-condition-view">
    <div class="page-header d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
      <div class="header-text">
        <h2 class="page-title mb-1">Monitoring Condition (Diagnosa)</h2>
        <p class="page-subtitle text-secondary mb-0">Sinkronisasi diagnosa ICD-10 pasien ke SatuSehat</p>
      </div>
    </div>
    ...
    <div class="card border-0 shadow-sm mb-4 animate__animated animate__fadeInDown">
      <div class="card-body">
        <div class="row g-3">
          ...
          <div class="col-md-3">
            <label class="form-label text-xs fw-bold text-uppercase text-muted mb-2 tracking-wide">Cari Pasien / ICD-10</label>
            <input type="text" class="form-control form-control-custom" v-model="filters.keyword" placeholder="Nama / NIK / Kode ICD..." @keyup.enter="fetchData(true)">
          </div>
            ...
            <tbody>
              <tr v-if="loading">
                <td colspan="5" class="text-center py-5">
                  ...
                  <p class="text-muted mt-2 mb-0">Memuat data diagnosa...</p>
                </td>
              </tr>
              <tr v-else-if="items.length === 0">
                <td colspan="5" class="text-center py-5">
                  <i class="fas fa-inbox fa-3x text-muted mb-3 opacity-50"></i>
                  <h6 class="text-muted">Tidak ada data diagnosa ditemukan</h6>
                </td>
              </tr>
              <tr v-for="(item, index) in items" :key="index" v-else>
                ...
                <td>
                  ...
                  <p class="mb-0 text-sm fw-medium">{{ item.nm_penyakit }}</p>
                  <span class="text-xs text-muted">Prioritas: {{ item.prioritas == '1' ? 'Utama' : 'Tambahan' }}</span>
                </td>
                <td>
                  <span :class="['badge', item.status === 'Ranap' ? 'bg-info' : 'bg-warning text-dark']">
                    {{ item.status }}
                  </span>
                </td>
...
export default {
  ...
  methods: {
    async fetchData(resetPage = false) {
      ...
      this.loading = true;
      try {
        const response = await satuSehatService.getConditions({
    ...
    async handleSync() {
      const result = await Swal.fire({
        ...
        text: `Kirim data diagnosa ICD-10 periode ${this.filters.tglAwal} s/d ${this.filters.tglAkhir}?`,
        icon: 'question',
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await satuSehatService.syncCondition({
            tglAwal: this.filters.tglAwal,
            tglAkhir: this.filters.tglAkhir
          });
          if (response.data && response.data.success) {
