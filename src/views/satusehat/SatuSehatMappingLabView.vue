<template>
  <div class="satusehat-mapping-lab">
    <div class="page-header d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
      <div class="header-text">
        <h2 class="page-title mb-1 text-primary-emphasis fw-bold">Mapping Laboratorium SatuSehat</h2>
        <p class="page-subtitle text-secondary mb-0">Hubungkan pemeriksaan laboratorium lokal dengan standar LOINC & SNOMED CT</p>
      </div>
        ...
        <div class="table-responsive">
          <table class="table table-hover align-middle mb-0 custom-table">
            ...
            <tbody>
              ...
              <tr v-for="proc in procedures" :key="proc.id_template">
                <td class="px-4">
                  <div class="fw-bold text-dark">{{ proc.Pemeriksaan }}</div>
                  <div class="text-xs text-muted">ID: {{ proc.id_template }}</div>
                </td>
                <td>
                  <div v-if="proc.satu_sehat_mapping_lab" class="mapping-badge loinc-badge">
                    <div class="badge-content">
                      <span class="badge-code font-mono">{{ proc.satu_sehat_mapping_lab.code }}</span>
                      <span class="badge-display">{{ proc.satu_sehat_mapping_lab.display }}</span>
                    </div>
                  </div>
                  <span v-else class="text-xs text-danger italic"><i class="fas fa-times-circle me-1"></i> Belum Mapping</span>
                </td>
                <td>
                  <div v-if="proc.satu_sehat_mapping_lab" class="mapping-badge snomed-badge">
                    <div class="badge-content">
                      <span class="badge-code font-mono">{{ proc.satu_sehat_mapping_lab.sampel_code }}</span>
                      <span class="badge-display">{{ proc.satu_sehat_mapping_lab.sampel_display }}</span>
                    </div>
                  </div>
                ...
                <td class="px-4 text-end">
                  <div class="btn-group shadow-sm">
                    ...
                    <button 
                      v-if="proc.satu_sehat_mapping_lab" 
                      class="btn btn-outline-danger btn-sm rounded-end-3 px-3" 
                      @click="deleteMapping(proc.id_template)"
                    >
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
          <div class="modal-header bg-primary text-white p-4 border-0">
            <div>
              <h5 class="modal-title fw-bold mb-0">Mapping Standar SatuSehat Lab</h5>
              <p class="mb-0 text-white-50 text-xs">Pemeriksaan: {{ formData.Pemeriksaan }}</p>
            </div>
          ...
          <div class="modal-body p-4 bg-light">
            <div class="ai-suggestion-section mb-4">
              <div class="card border-0 shadow-sm rounded-4 overflow-hidden bg-white">
                <div class="card-body p-4">
                  <div class="d-flex justify-content-between align-items-center mb-3">
                    <div class="d-flex align-items-center gap-3">
                      ...
                      <h6 class="mb-0 fw-bold">AI Search Agent (Lab)</h6>
                    </div>
                    <button 
                  ...
                  <p class="text-muted small mb-0">Gunakan AI untuk mencari kode LOINC dan SNOMED CT yang paling relevan berdasarkan spesimen laboratorium.</p>
                ...
                <div v-if="aiResults.length > 0" class="mt-3 text-center">
                  <button 
                    class="btn btn-outline-indigo btn-sm rounded-pill px-4" 
                    @click="searchAI(true)" 
                    :disabled="loadingAI"
                  >
                    <i v-if="loadingAI" class="fas fa-spinner fa-spin me-2"></i>
            ...
            <form @submit.prevent="saveMapping">
              <div class="row g-4 overflow-y-auto" style="max-height: 500px; padding: 5px;">
                ...
                <div class="col-md-6">
                  <div class="card border-0 shadow-sm rounded-3">
                    <div class="card-header bg-white py-3">
                      <h6 class="mb-0 fw-bold"><i class="fas fa-tag text-primary me-2"></i> LOINC (Lab Parameter)</h6>
                    </div>
                    <div class="card-body">
                      <div class="mb-3">
                ...
                <div class="col-md-6">
                  <div class="card border-0 shadow-sm rounded-3">
                    <div class="card-header bg-white py-3">
                      <h6 class="mb-0 fw-bold"><i class="fas fa-vial text-success me-2"></i> Specimen (SNOMED CT)</h6>
                    </div>
                    <div class="card-body">
                      <div class="mb-3">
                        <label class="form-label text-xs fw-bold text-uppercase">Specimen Code</label>
                        <input type="text" class="form-control" v-model="formData.sampel_code" placeholder="e.g. 119361006" required />
                      </div>
                      <div class="mb-3">
                        <label class="form-label text-xs fw-bold text-uppercase">Specimen Display</label>
                        <textarea class="form-control" v-model="formData.sampel_display" rows="2" placeholder="Specimen Type Name" required></textarea>
                      </div>
                      <div class="mb-0">
                        <label class="form-label text-xs fw-bold text-uppercase">Specimen System</label>
  ...
  methods: {
    async fetchProcedures() {
      ...
      try {
        ...
        const res = await satuSehatService.getMappingLab(params)
        if (res.data.success) {
          ...
          this.mappedCount = this.procedures.filter(p => p.satu_sehat_mapping_lab).length
          if (this.filterStatus === 'mapped') this.mappedCount = this.totalProcedures
        }
      } catch (err) {
        console.error('Fetch error:', err)
        Swal.fire({ icon: 'error', title: 'Error', text: 'Gagal mengambil data pemeriksaan laboratorium' })
      } finally {
        this.loading = false
      }
    },
    ...
    openModal(proc) {
      this.formData = {
        id_template: proc.id_template,
        Pemeriksaan: proc.Pemeriksaan,
        code: proc.satu_sehat_mapping_lab?.code || '',
        system: 'http://loinc.org',
        display: proc.satu_sehat_mapping_lab?.display || '',
        sampel_code: proc.satu_sehat_mapping_lab?.sampel_code || '',
        sampel_system: 'http://snomed.info/sct',
        sampel_display: proc.satu_sehat_mapping_lab?.sampel_display || ''
      }
      this.aiResults = []
      this.aiPage = 1
    ...
    async saveMapping() {
      ...
      try {
        const res = await satuSehatService.saveMappingLab(this.formData)
        if (res.data.success) {
    ...
    async deleteMapping(id) {
      const confirm = await Swal.fire({
        title: 'Hapus Mapping?',
        text: 'Data mapping laboratorium akan dihapus permanen!',
        icon: 'warning',
      ...
      if (confirm.isConfirmed) {
        try {
          const res = await satuSehatService.deleteMappingLab(id)
          if (res.data.success) {
    ...
    async searchAI(isNext = false) {
      ...
      try {
        const res = await satuSehatService.searchAiLab({
          query: this.formData.Pemeriksaan,
          page: this.aiPage
        })
        ...
        if (results.length === 0) {
          if (isNext) {
            Swal.fire({ icon: 'info', title: 'Selesai', text: 'Tidak ada rekomendasi tambahan lagi' })
            this.aiPage-- // Revert page
          } else {
            Swal.fire({ icon: 'info', title: 'No Results', text: 'AI tidak menemukan rekomendasi yang cocok' })
          }
        } else {
          if (isNext) {
            this.aiResults = [...this.aiResults, ...results]
          } else {
