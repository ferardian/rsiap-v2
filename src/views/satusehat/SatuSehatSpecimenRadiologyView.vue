<template>
  <div class="satusehat-specimen-radiology-view">
    <div class="page-header d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
      <div class="header-text">
        <h2 class="page-title mb-1">Monitoring Specimen Radiologi</h2>
        <p class="page-subtitle text-secondary mb-0">Sinkronisasi data spesimen pemeriksaan radiologi ke SatuSehat</p>
      </div>
    </div>
            ...
            <tbody>
              <tr v-if="loading">
                <td colspan="5" class="text-center py-5">
                  ...
                  <p class="text-muted mt-2 mb-0">Memuat data specimen...</p>
                </td>
              </tr>
              <tr v-else-if="items.length === 0">
                <td colspan="5" class="text-center py-5">
                  <i class="fas fa-box-open fa-3x text-muted mb-3 opacity-50"></i>
                  <h6 class="text-muted">Tidak ada data specimen radiologi ditemukan</h6>
                </td>
              </tr>
              <tr v-for="(item, index) in items" :key="index" v-else>
                ...
                <td>
                  <span class="d-block fw-medium text-dark mb-1">{{ item.nm_perawatan }}</span>
                  <span class="badge bg-soft-info text-info border border-info border-opacity-10 text-xs">
                    <i class="fas fa-vial me-1"></i> {{ item.sampel_display }}
                  </span>
                  <div class="mt-2">
                    <span v-if="item.id_servicerequest" class="badge bg-soft-success text-success text-xs border border-success border-opacity-25" :title="item.id_servicerequest">
                      <i class="fas fa-link me-1"></i> ServiceRequest
                    </span>
                    <span v-else class="badge bg-soft-danger text-danger text-xs border border-danger border-opacity-25">
                      <i class="fas fa-unlink me-1"></i> No ServiceRequest
                    </span>
                  </div>
                </td>
                <td>
                  <div class="d-flex flex-column gap-1">
                    <span class="text-sm fw-medium text-dark">{{ item.tgl_sampel }}</span>
                    <span class="text-xs text-muted">{{ item.jam_sampel }}</span>
                  </div>
                </td>
                <td class="text-center">
                  <div v-if="item.id_specimen" class="badge-success-glow" :title="'ID: ' + item.id_specimen">
...
export default {
  name: 'SatuSehatSpecimenRadiologyView',
  data() {
    return {
      items: [],
      loading: false,
  ...
  methods: {
    async fetchData(resetPage = false) {
      ...
      this.loading = true;
      try {
        const response = await satuSehatService.getSpecimenRadiology({
    ...
    async handleSync() {
      const result = await Swal.fire({
        ...
        text: `Kirim data Specimen Radiologi periode ${this.filters.tglAwal} s/d ${this.filters.tglAkhir}?`,
        icon: 'question',
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await satuSehatService.syncSpecimenRadiology({
            tglAwal: this.filters.tglAwal,
            tglAkhir: this.filters.tglAkhir
          });
          if (response.data && response.data.success) {
