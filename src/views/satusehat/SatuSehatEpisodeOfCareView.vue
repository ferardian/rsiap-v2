<template>
  <div class="satusehat-episodeofcare-view">
    <div class="page-header d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
      <div class="header-text">
        <h2 class="page-title mb-1">Monitoring EpisodeOfCare (ANC)</h2>
        <p class="page-subtitle text-secondary mb-0">Manajemen episode kehamilan pasien ke SatuSehat</p>
      </div>
    </div>
      ...
      <div class="card-body">
        <div class="row g-3">
          ...
          <div class="col-md-3">
            ...
            <input type="text" class="form-control form-control-custom" v-model="filters.keyword" placeholder="Nama / RM / No. Rawat..." @keyup.enter="fetchData(true)">
          </div>
          ...
          <table class="table table-hover table-custom mb-0">
            <thead>
              <tr>
                ...
                <th>Pasien</th>
                <th>Diagnosa Terkait (ANC)</th>
                <th class="text-center">Status Condition</th>
                <th class="text-center">Status EpisodeOfCare</th>
              </tr>
            </thead>
            <tbody>
              <tr v-if="loading">
                <td colspan="5" class="text-center py-5">
                  ...
                  <p class="text-muted mt-2 mb-0">Memuat data EpisodeOfCare...</p>
                </td>
              </tr>
              <tr v-else-if="items.length === 0">
                <td colspan="5" class="text-center py-5">
                  <i class="fas fa-inbox fa-3x text-muted mb-3 opacity-50"></i>
                  <h6 class="text-muted">Tidak ada data ANC ditemukan untuk periode ini</h6>
                </td>
              </tr>
              <tr v-for="(item, index) in items" :key="index" v-else>
                ...
                <td>
                  <span class="badge bg-soft-info text-info fw-bold mb-1">{{ item.kd_penyakit }}</span>
                  <p class="mb-0 text-sm fw-medium text-truncate" style="max-width: 250px;">{{ item.nm_penyakit }}</p>
                </td>
                <td class="text-center">
                  <div v-if="item.id_condition" class="badge-success-glow" :title="'ID: ' + item.id_condition">
                    <i class="fas fa-check-circle me-1"></i> Terkirim
                  </div>
                  <div v-else class="badge-warning-glow">
                    <i class="fas fa-exclamation-triangle me-1"></i> Condition Missing
                  </div>
                </td>
                <td class="text-center">
                  <div v-if="item.id_episode_of_care" class="badge-success-glow" :title="'ID: ' + item.id_episode_of_care">
                    <i class="fas fa-check-circle me-1"></i> {{ item.status_eoc || 'Active' }}
                  </div>
                  <div v-else class="badge-warning-glow">
...
export default {
  name: 'SatuSehatEpisodeOfCareView',
  data() {
    return {
      items: [],
      loading: false,
  ...
  methods: {
    async fetchData(resetPage = false) {
      ...
      this.loading = true;
      try {
        const response = await satuSehatService.getEpisodeOfCare({
        ...
        console.error('Error fetching EpisodeOfCare:', error);
      } finally {
        this.loading = false;
      }
    },
    ...
    async handleSync() {
      const result = await Swal.fire({
        ...
        text: `Kirim data EpisodeOfCare (ANC) periode ${this.filters.tglAwal} s/d ${this.filters.tglAkhir}?`,
        icon: 'question',
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await satuSehatService.syncEpisodeOfCare({
            tglAwal: this.filters.tglAwal,
            tglAkhir: this.filters.tglAkhir
          });
          if (response.data && response.data.success) {
