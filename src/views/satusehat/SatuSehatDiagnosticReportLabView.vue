<template>
  <div class="satusehat-diagnosticreport-lab">
      ...
      <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div class="header-text">
          <div class="d-flex align-items-center gap-2 mb-1">
            ...
            <h2 class="page-title mb-0">DiagnosticReport Lab <span class="text-primary-emphasis">SatuSehat</span></h2>
          </div>
          <p class="page-subtitle text-muted mb-0">Sinkronisasi dokumen laporan hasil laboratorium (DiagnosticReport) ke platform SatuSehat FHIR</p>
        </div>
      ...
      <div class="col-md-3 min-w-stats">
        <div class="card stat-card shadow-sm border-0 h-100 overflow-hidden">
          <div class="stat-card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <p class="stat-label mb-0">Total Report</p>
        ...
        <div class="card stat-card shadow-sm border-0 h-100 overflow-hidden">
          <div class="stat-card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              ...
              <h3 class="stat-value mb-0">{{ items.filter(i => i.id_diagnosticreport).length }}</h3>
            </div>
          </div>
          ...
          <table class="table table-hover align-middle mb-0 custom-table-premium">
            ...
            <tbody>
              ...
              <tr v-else-if="items.length === 0">
                <td colspan="7" class="py-5 text-center no-data-state">
                  <div class="empty-state-icon">
                    <i class="fas fa-folder-open"></i>
                  </div>
                  <p class="mt-3 fw-medium text-secondary">No DiagnosticReport records found</p>
              ...
              <tr v-for="item in items" :key="item.noorder + item.id_template" class="hover-row">
                ...
                <td>
                  <div v-if="item.id_observation" class="sync-status success mini">
                    <div class="sync-icon"><i class="fas fa-microscope"></i></div>
                    <div class="sync-text">
                  ...
                  <div v-else class="sync-status danger mini">
                    ...
                    <div class="sync-text">
                      <span class="status-label">Missing Obs</span>
                    </div>
                  </div>
                </td>
                <td>
                ...
                <td>
                  <transition name="scale" mode="out-in">
                    <div v-if="item.id_diagnosticreport" class="sync-status success" :key="'sent-' + item.noorder">
                      <div class="sync-icon"><i class="fas fa-check"></i></div>
                      <div class="sync-text">
                        <span class="status-label">Synced</span>
                        <span class="status-id">{{ item.id_diagnosticreport }}</span>
                      </div>
                    </div>
                ...
                <td class="pe-4 text-end">
                  <div class="action-buttons">
                    <button 
                      ...
                      :class="item.id_diagnosticreport ? 'btn-update' : 'btn-send'"
                      @click="sendToSatuSehat(item)"
                      :disabled="sendingItems.includes(item.noorder + item.id_template) || !item.no_ktp || !item.id_observation || !item.id_specimen"
                    >
                      ...
                      <template v-else>
                        <i :class="item.id_diagnosticreport ? 'fas fa-sync' : 'fas fa-paper-plane'"></i>
                        {{ item.id_diagnosticreport ? 'Refetch' : 'Send Report' }}
                      </template>
                    </button>
                  </div>
                </td>
...
export default {
  name: 'SatuSehatDiagnosticReportLabView',
  data() {
    return {
      items: [],
      loading: false,
  ...
  methods: {
    async fetchData() {
      this.loading = true
      try {
        const res = await satuSehatService.getDiagnosticReportLab(this.filters)
        if (res.data.success) {
        ...
        Swal.fire({ 
          ...
          text: err.response?.data?.message || 'Gagal mengambil data dari server SIMRS.' 
        })
      } finally {
        this.loading = false
      }
    ...
    async syncBatch() {
      const result = await Swal.fire({
        ...
        text: `Sistem akan mengirimkan seluruh DiagnosticReport Lab yang belum terkirim pada periode registrasi ${this.filters.tgl_awal} s/d ${this.filters.tgl_akhir} ke SatuSehat.`,
        icon: 'question',
      ...
      this.syncing = true
      try {
        const res = await satuSehatService.syncDiagnosticReportLab({
    ...
    async sendToSatuSehat(item) {
      ...
      try {
        const res = await satuSehatService.sendDiagnosticReportLab(item)
        if (res.data.success) {
          Swal.fire({ 
            icon: 'success', 
...
.satusehat-diagnosticreport-lab {
  padding-bottom: 2rem;
}
...
.stat-icon-wrapper.primary { background: #ecfdf5; color: #10b981; }
.stat-icon-wrapper.success { background: #f0fdf4; color: #16a34a; }
.stat-icon-wrapper.warning { background: #fffbeb; color: #d97706; }
...
.stat-wave.primary { background: #10b981; }
.stat-wave.success { background: #16a34a; }
.stat-wave.warning { background: #d97706; }
...
.btn-search-go:hover { background: #059669; transform: scale(1.05); }
...
.reg-number { font-size: 0.7rem; font-weight: 600; color: #10b981; }
...
.exam-name { font-weight: 700; color: #059669; font-size: 0.85rem; }
...
.sync-status.warning .sync-icon { background: #d97706; color: white; }
...
.sync-status.success .status-label { color: #059669; }
...
.btn-send { background: #10b981; color: white; border: none; }
.btn-send:hover:not(:disabled) { background: #059669; transform: translateY(-2px); box-shadow: 0 4px 12px rgba(16, 185, 129, 0.4); }

.btn-update { background: white; color: #10b981; border: 1px solid #10b981; }
.btn-update:hover:not(:disabled) { background: #10b981; color: white; transform: translateY(-2px); box-shadow: 0 4px 12px rgba(16, 185, 129, 0.2); }
