<template>
  <div class="satusehat-procedure-view">
    <div class="page-header d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
      <div class="header-text">
        <h2 class="page-title mb-1">Monitoring Procedure (Tindakan)</h2>
        <p class="page-subtitle text-secondary mb-0">Sinkronisasi tindakan medis ICD-9-CM pasien ke SatuSehat</p>
      </div>
    </div>
    ...
    <div class="card border-0 shadow-sm mb-4 animate__animated animate__fadeInDown">
      <div class="card-body">
        <div class="row g-3">
          ...
          <div class="col-md-3">
            <label class="form-label text-xs fw-bold text-uppercase text-muted mb-2 tracking-wide">Cari Pasien / ICD-9</label>
            <input type="text" class="form-control form-control-custom" v-model="filters.keyword" placeholder="Nama / No.Rawat / Kode..." @keyup.enter="fetchData(true)">
          </div>
            ...
            <tbody>
              <tr v-if="loading">
                <td colspan="5" class="text-center py-5">
                  ...
                  <p class="text-muted mt-2 mb-0">Memuat data prosedur...</p>
                </td>
              </tr>
              <tr v-else-if="items.length === 0">
                <td colspan="5" class="text-center py-5">
                  <i class="fas fa-clipboard-list fa-3x text-muted mb-3 opacity-50"></i>
                  <h6 class="text-muted">Tidak ada data prosedur ditemukan</h6>
                </td>
              </tr>
              <tr v-for="(item, index) in items" :key="index" v-else>
                ...
                <td>
                  <span class="badge bg-soft-primary text-primary fw-bold mb-1">{{ item.kode }}</span>
                  <p class="mb-0 text-sm fw-medium line-clamp-2" :title="item.nm_prosedur">{{ item.nm_prosedur }}</p>
                </td>
                <td>
                ...
                <td class="text-center">
                  <div v-if="item.id_procedure" class="badge-success-glow" :title="'ID: ' + item.id_procedure">
...
export default {
  ...
  methods: {
    async fetchData(resetPage = false) {
      ...
      this.loading = true;
      try {
        const response = await satuSehatService.getProcedures({
    ...
    async handleSync() {
      const result = await Swal.fire({
        ...
        text: `Kirim data prosedur ICD-9-CM periode ${this.filters.tglAwal} s/d ${this.filters.tglAkhir}?`,
        icon: 'question',
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await satuSehatService.syncProcedure({
            tglAwal: this.filters.tglAwal,
            tglAkhir: this.filters.tglAkhir
          });
          if (response.data && response.data.success) {
