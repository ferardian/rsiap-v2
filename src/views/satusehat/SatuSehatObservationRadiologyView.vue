<template>
  <div class="satusehat-observation-radiology-view">
    <div class="page-header d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
      <div class="header-text">
        <h2 class="page-title mb-1">Monitoring Observation Radiologi</h2>
        <p class="page-subtitle text-secondary mb-0">Sinkronisasi hasil ekspertise pemeriksaan radiologi ke SatuSehat</p>
      </div>
    </div>
            ...
            <tbody>
              ...
              <tr v-else-if="items.length === 0">
                <td colspan="5" class="text-center py-5">
                  ...
                  <h6 class="text-muted">Tidak ada data observation radiologi ditemukan</h6>
                </td>
              </tr>
              <tr v-for="(item, index) in items" :key="index" v-else>
                ...
                <td>
                  ...
                  <div class="text-truncate text-muted text-xs mb-2" style="max-width: 300px;" :title="item.hasil">
                    {{ item.hasil }}
                  </div>
                ...
                <td class="text-center">
                  <div v-if="item.id_observation" class="badge-success-glow" :title="'ID: ' + item.id_observation">
...
export default {
  name: 'SatuSehatObservationRadiologyView',
  data() {
    return {
      items: [],
      loading: false,
  ...
  methods: {
    async fetchData(resetPage = false) {
      ...
      this.loading = true;
      try {
        const response = await satuSehatService.getObservationRadiology({
    ...
    async handleSync() {
      const result = await Swal.fire({
        ...
        text: `Kirim data Observation Radiologi (Hasil Ekspertise) periode ${this.filters.tglAwal} s/d ${this.filters.tglAkhir}?`,
        icon: 'question',
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await satuSehatService.syncObservationRadiology({
            tglAwal: this.filters.tglAwal,
            tglAkhir: this.filters.tglAkhir
          });
          if (response.data && response.data.success) {
