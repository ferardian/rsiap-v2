  ...
  <div class="card border-0">
    <div class="card-body p-0">
          ...
          <tbody>
            ...
            <tr v-else v-for="(item, index) in items" :key="item.id_inmut">
              ...
              <td>
                ...
                <div class="inmut-def text-truncate" style="max-width: 350px;" :title="stripHtml(item.definisi_operasional)">{{ stripHtml(item.definisi_operasional) }}</div>
              </td>
              <td>
                <span class="badge-ruang">{{ item.nama_ruang || item.dep_id }}</span>
              </td>
              ...
              <td class="fw-semibold text-slate-700 small">{{ getRumusSymbol(item.rumus) }} {{ item.standar }} {{ item.satuan }}</td>
              <td class="text-center">
      ...
      <div class="d-md-none p-3">
        ...
        <div v-else v-for="(item, index) in items" :key="item.id_inmut" class="card mb-3 border shadow-sm rounded-3 overflow-hidden">
          <div class="card-body p-3">
            <div class="d-flex justify-content-between align-items-start mb-2">
              <div>
                <span class="badge bg-primary-subtle text-primary border small me-1">{{ item.nama_ruang || item.dep_id }}</span>
                <span class="badge bg-light text-dark border small">{{ item.nama_jenis }}</span>
              </div>
            ...
            <p class="text-muted small mb-3 text-truncate-2">{{ stripHtml(item.definisi_operasional) }}</p>
            
            <div class="bg-light p-2 rounded-3 mb-3">
               <small class="text-muted d-block">Standar</small>
               <span class="fw-bold">{{ getRumusSymbol(item.rumus) }} {{ item.standar }} {{ item.satuan }}</span>
            </div>
