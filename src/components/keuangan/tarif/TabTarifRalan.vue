            ...
            <div class="d-flex align-items-center gap-2.5">
              ...
              <div>
                <h6 class="modal-title fw-bold text-dark m-0">
                  {{ modalMode === 'add' ? 'Tambah Tarif Rawat Jalan' : 'Edit Tarif Rawat Jalan' }}
                </h6>
                <p class="text-muted fs-xs m-0 mt-0.5">Lengkapi data tindakan dan rincian tarif perawatan</p>
              </div>
            </div>
            ...
            <form @submit.prevent="saveData">
              ...
              <div class="card border rounded-3 p-3 mb-3 bg-white shadow-none position-relative" style="z-index: 15; overflow: visible !important;">
                <div class="fs-xs fw-bold text-secondary text-uppercase tracking-wider border-bottom pb-1.5 mb-2.5 d-flex align-items-center gap-1.5">
                  <i class="fas fa-layer-group text-muted"></i> Informasi Tindakan
                </div>
                <div class="row g-2.5">
                  <div class="col-md-4">
                  ...
                  <div class="col-md-6">
                    <label class="form-label fs-xs fw-semibold text-secondary mb-1">Poliklinik</label>
                    <v-select
                      v-model="form.kd_poli"
                      :options="listPoli"
                      label="nm_poli"
                      :reduce="poli => poli.kd_poli"
                      :filter-by="filterPoli"
                      :clearable="false"
                      placeholder="Pilih Poliklinik..."
                      class="v-select-custom"
                    >
                      <template #option="{ nm_poli, kd_poli }">
                        <div class="d-flex justify-content-between align-items-center py-0.5">
                          <span class="fs-xs fw-medium text-dark">{{ nm_poli }}</span>
                          <span class="badge bg-light text-secondary border font-monospace fs-xxs ms-2">{{ kd_poli }}</span>
                        </div>
                      </template>
                      <template #selected-option="{ nm_poli, kd_poli }">
                        <span class="fs-xs fw-medium text-dark">{{ nm_poli }} <small class="text-muted">({{ kd_poli }})</small></span>
                      </template>
                    </v-select>
                  </div>
                </div>
...
const filterPoli = (option, label, search) => {
  const s = (search || '').toLowerCase()
  return (option.nm_poli || '').toLowerCase().includes(s) ||
         (option.kd_poli || '').toLowerCase().includes(s)
}
...
const openModal = async (mode, item = null) => {
  ...
  if (listPoli.value.length === 0) fetchPoli()
