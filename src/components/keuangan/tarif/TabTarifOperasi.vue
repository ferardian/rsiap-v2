          ...
          <table class="table align-middle">
            <thead>
              <tr>
                ...
                <th class="text-end">Tarif Operator 1</th>
                <th class="text-end">Tarif Sewa OK/VK</th>
            ...
            <tbody>
              ...
              <tr v-for="item in items" :key="item.kode_paket" class="hover-row">
                ...
                <td class="text-end numeric-text">
                  {{ formatRupiah(item.operator1) }}
                </td>
                <td class="text-end numeric-text">
                  {{ formatRupiah(item.sewa_ok || item.akomodasi) }}
                </td>
...
const fetchData = async () => {
  ...
  try {
    ...
    console.error('Error fetching tarif operasi', error)
    if (error.response?.status === 404) {
       items.value = [
          { kode_paket: 'OP001', nm_perawatan: 'SC (Sectio Caesarea)', kategori: 'Kebidanan', kelas: 'Kelas 1', operator1: 2500000, sewa_ok: 1000000, status: '1' },
          { kode_paket: 'OP002', nm_perawatan: 'Appendiktomi', kategori: 'Operasi', kelas: 'Kelas 3', operator1: 2000000, sewa_ok: 800000, status: '1' }
       ]
    } else {
       toast.error('Gagal mengambil data tarif operasi')
    }
  } finally {
    loading.value = false
  }
