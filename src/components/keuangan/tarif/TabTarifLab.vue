...
const getKategoriName = (kategori) => {
  if (kategori === 'PK') return 'Patologi Klinik'
  if (kategori === 'PA') return 'Patologi Anatomi'
  if (kategori === 'MB') return 'Mikrobiologi'
  return kategori
}
...
const fetchData = async () => {
  ...
  try {
    ...
    console.error('Error fetching tarif lab', error)
    if (error.response?.status === 404) {
       items.value = [
          { kd_jenis_prw: 'L001', nm_perawatan: 'Darah Lengkap', kategori: 'PK', kelas: 'Rawat Jalan', tarif_tindakan_dokter: 20000, tarif_tindakan_petugas: 15000, total_byr: 85000, status: '1' },
          { kd_jenis_prw: 'L002', nm_perawatan: 'Urine Lengkap', kategori: 'PK', kelas: 'Kelas 3', tarif_tindakan_dokter: 15000, tarif_tindakan_petugas: 10000, total_byr: 55000, status: '1' }
       ]
    } else {
